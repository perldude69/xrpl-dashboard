   ...
   function createPanel(config) {
    ...
    function loadAndCreatePanels() {
     ...
     panels.forEach(config => {
      ...
      // XRP Payments accumulator for this panel (per-ledger sum)
      let currentLedgerForPanel = null;
      let xrpPaymentsSum = 0;
      ...
      window.XRPL.socket.on(`panelTransaction:${config.id}`, (data) => {
        ...
        // XRP payments accumulator per ledger
        if (data.ledger !== currentLedgerForPanel) {
          currentLedgerForPanel = data.ledger;
          xrpPaymentsSum = 0;
        }
        ...
        const xrpElem = document.getElementById('xrpPayments');
        if (xrpElem) {
          xrpElem.textContent = xrpPaymentsSum.toFixed(6);
        }

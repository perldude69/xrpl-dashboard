...
function setupWalletSocket(io, userData) {
    ...
    socket.on('setWatchedAddresses', async (addresses) => {
      ...
      try {
        ...
        getLatestPrice((err, price) => {
          if (err || !price) price = null;
          const balancesWithPrice = balances.map(bal => ({
            ...bal,
            usdValue: (typeof bal.balance === 'number' && price) ? (bal.balance * price).toFixed(2) : 'N/A'
          }));
          socket.emit('balances', balancesWithPrice);
        });
...
module.exports = { setupWalletSocket };